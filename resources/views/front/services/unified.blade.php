...
.svc-card.on{border-color:rgba(var(--onx-brand-rgb),.6);background:rgba(var(--onx-brand-rgb),.1);box-shadow:0 8px 28px rgba(var(--onx-brand-rgb),.12);}
.svc-check{background:var(--onx-brand);}
.det-stat-v{color:var(--onx-brand);}
.det-badge{background:rgba(var(--onx-brand-rgb),.1);border:1px solid rgba(var(--onx-brand-rgb),.22);color:rgba(var(--onx-brand-rgb),.85);}
.pkg:hover .pkg-line,.pkg.feat .pkg-line{background:var(--onx-brand);}
.pkg.feat{border-color:rgba(var(--onx-brand-rgb),.45);background:rgba(var(--onx-brand-rgb),.08);}
.pkg-pr{color:var(--onx-brand);}
.pkg-feats li::before{color:var(--onx-brand);}
.pkg-btn:hover{background:rgba(var(--onx-brand-rgb),.15);border-color:rgba(var(--onx-brand-rgb),.4);color:var(--onx-brand);}
.pkg.feat .pkg-btn{background:var(--onx-brand);border-color:var(--onx-brand);}
.testi-stars{color:var(--onx-brand);}
.testi-av{background:rgba(var(--onx-brand-rgb),.18);border:1px solid rgba(var(--onx-brand-rgb),.25);color:var(--onx-brand);}
.btn-book-main{background:var(--onx-brand);box-shadow:0 4px 20px rgba(var(--onx-brand-rgb),.28);}
.bottom-bar{border-top:1px solid rgba(var(--onx-brand-rgb),.2);}
.bb-price{color:var(--onx-brand);}
.bb-btn{background:var(--onx-brand);}
...
.work-cat{color:rgba(var(--onx-brand-rgb),.8);}
...
.svc-card{background:var(--glass);border:1px solid var(--border);border-radius:14px;padding:18px 12px;text-align:center;cursor:pointer;transition:all .25s;position:relative;overflow:hidden;}
.svc-card::after{content:'';position:absolute;bottom:0;left:0;right:0;height:2px;background:var(--onx-brand);transform:scaleX(0);transition:transform .3s;}
.svc-card:hover{border-color:rgba(var(--onx-brand-rgb),.3);background:rgba(var(--onx-brand-rgb),.05);transform:translateY(-2px);}
.svc-card:hover::after,.svc-card.on::after{transform:scaleX(1);}
.svc-card.on{border-color:rgba(var(--onx-brand-rgb),.6);background:rgba(var(--onx-brand-rgb),.1);transform:translateY(-3px);box-shadow:0 8px 28px rgba(var(--onx-brand-rgb),.12);}
.svc-ico{font-size:26px;margin-bottom:8px;display:block;}
.svc-nm{font-size:12px;font-weight:800;color:#fff;margin-bottom:3px;}
...
.det-head{padding:28px 32px 24px;border-bottom:1px solid rgba(255,255,255,.06);display:grid;grid-template-columns:1fr auto;gap:24px;align-items:start;}
@media(max-width:680px){.det-head{grid-template-columns:1fr;}}
.det-badge{display:inline-flex;align-items:center;gap:5px;background:rgba(var(--onx-brand-rgb),.1);border:1px solid rgba(var(--onx-brand-rgb),.22);border-radius:99px;padding:3px 10px;font-size:9px;font-weight:800;color:rgba(var(--onx-brand-rgb),.85);margin-bottom:10px;}
.det-title{font-size:clamp(1.4rem,2.8vw,2rem);font-weight:900;color:#fff;margin-bottom:8px;}
.det-desc{font-size:13px;color:rgba(255,255,255,.5);line-height:1.9;max-width:500px;}
.det-stats{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;min-width:220px;}
.det-stat{background:rgba(255,255,255,.04);border:1px solid var(--border);border-radius:12px;padding:12px 8px;text-align:center;}
.det-stat-v{font-size:18px;font-weight:900;color:var(--onx-brand);line-height:1;}
.det-stat-l{font-size:8px;color:var(--muted);margin-top:3px;}
...
.pkg-line{position:absolute;top:0;left:0;right:0;height:3px;background:rgba(255,255,255,.08);transition:background .3s;}
.pkg:hover{border-color:rgba(var(--onx-brand-rgb),.3);transform:translateY(-3px);box-shadow:0 12px 32px rgba(0,0,0,.3);}
...
.pkg-btn{width:100%;background:rgba(255,255,255,.07);border:1px solid rgba(255,255,255,.1);border-radius:99px;padding:10px;font-size:11px;font-weight:800;color:rgba(255,255,255,.7);cursor:pointer;font-family:inherit;transition:all .2s;margin-top:auto;}
.pkg-btn:hover{background:rgba(var(--onx-brand-rgb),.15);border-color:rgba(var(--onx-brand-rgb),.4);color:var(--onx-brand);}
...
.work-overlay{position:absolute;inset:0;background:linear-gradient(to top,rgba(0,0,0,.85),transparent 55%);display:flex;align-items:flex-end;padding:10px;}
...
.testi-txt{font-size:11px;color:rgba(255,255,255,.55);line-height:1.8;margin-bottom:10px;}
.testi-usr{display:flex;align-items:center;gap:8px;}
...
.bb-info strong{color:#fff;font-weight:800;}
                ...
                <div class="relative mx-auto max-w-md">
                    ...
                    <div class="border-spin relative overflow-hidden rounded-[28px] shadow-[0_32px_90px_rgba(0,0,0,.6)]">
                        ...
                        <div class="absolute bottom-0 left-0 right-0 p-5">
                            ...
                            <p class="mt-1.5 text-xs leading-6 text-white/60" x-text="activeCat().description||'من أول frame لآخر لقطة — كل شيء مدروس.'"></p>
                        </div>
                    </div>
                </div>
            </div>
...
<div class="cats-wrap">
    <div class="cats-inner">
        ...
        <template x-if="!initLoading">
            <template x-for="(cat,ci) in apiCats" :key="cat.id">
                <button type="button" class="cat-btn" :class="selCat===cat.id?'on':''" @click="pickCat(cat.id,ci)">
...
<div class="svcs-section">
    ...
    <template x-if="!loadingSvcs && services.length===0 && !initLoading">
        <div class="empty-svc"><div class="e-ico">📭</div><p>لا توجد خدمات في هذا التصنيف</p></div>
    </template>
    <div x-show="!loadingSvcs && services.length>0" class="svcs-grid">
        <template x-for="svc in services" :key="svc.id">
            <div class="svc-card" :class="selSvc===svc.id?'on':''" @click="pickSvc(svc)">
                <div class="svc-check">✓</div>
                <span class="svc-ico" x-text="svc.icon||'🎬'"></span>
                <div class="svc-nm" x-text="svc.name"></div>
                <div class="svc-ds" x-text="(svc.description||'').substring(0,55)"></div>
            </div>
        </template>
    </div>
</div>
...
<div class="det-section" id="svc-detail">
    ...
    <template x-if="!loadingDet && det">
        <div class="det-wrap">
            <div class="det-head">
                <div>
                    <div class="det-badge"><span x-text="activeCat().icon||'📌'"></span><span x-text="activeCat().name||''"></span></div>
                    <div class="det-title" x-text="det.name"></div>
                    <div class="det-desc" x-text="det.description||''"></div>
                </div>
                <div class="det-stats">
                    <div class="det-stat"><div class="det-stat-v" x-text="det.stats?.projects||'50+'"></div><div class="det-stat-l">مشروع</div></div>
                    <div class="det-stat"><div class="det-stat-v">5.0</div><div class="det-stat-l">تقييم</div></div>
                    <div class="det-stat"><div class="det-stat-v" x-text="det.stats?.delivery||'أسبوع'"></div><div class="det-stat-l">تسليم</div></div>
                </div>
            </div>
            <div class="det-body">
                <template x-if="det.packages && det.packages.length">
                    <div>
                        <div class="det-lbl"></div>
                            ...
                            <template x-for="pkg in det.packages" :key="pkg.id">
                                <div class="pkg" :class="pkg.is_featured?'feat':''">
                                    ...
                                    <button type="button" class="pkg-btn" @click="handleBookPkg(pkg)">
                                        <span x-text="pkg.price>0?'احجز هذه الباقة ←':'تواصل معنا'"></span>
                                    </button>
                                </div>
                            </template>
                        </div>
                ...
                <template x-if="det.portfolio && det.portfolio.length">
                    <div>
                        <div class="det-lbl">أعمال مرتبطة</div>
                        <div class="works-grid">
                            <template x-for="(item,i) in det.portfolio.slice(0,6)" :key="i">
                                <div class="work-card">
                                    <template x-if="item.thumb"><img :src="item.thumb" :alt="item.title||''" loading="lazy"></template>
                                    <template x-if="!item.thumb"><div class="work-ph">📷</div></template>
                                    <div class="work-overlay"><div><div class="work-cat" x-text="item.category||''"></div><div class="work-nm" x-text="item.title||''"></div></div></div>
                                </div>
                            </template>
                        </div>
                    </div>
                </template>
                <template x-if="det.testimonials && det.testimonials.length">
                    <div>
                        <div class="det-lbl">آراء العملاء</div>
                        <div class="testis-grid">
                            <template x-for="(t,ti) in det.testimonials.slice(0,4)" :key="ti">
                                <div class="testi">
                                    <div class="testi-stars" x-text="'★'.repeat(t.rating||5)"></div>
                                    <div class="testi-txt" x-text="'&quot;'+t.content+'&quot;'"></div>
                                    <div class="testi-usr">
                                        <div class="testi-av" x-text="(t.client_name||'ع').charAt(0)"></div>
                                        <div><div class="testi-nm" x-text="t.client_name||'عميل'"></div><div class="testi-rl" x-text="t.subtitle||t.client_role||''"></div></div>
                                    </div>
                                </div>
                            </template>
                        </div>
...
function servicesPage() {
    return {
        ...
        qbBusy: false,
        qbErr: '',
        qbForm: { name:'', phone:'', email:'', notes:'', event_date:'' },
        ...
        async pickCat(catId, i) {
            if (this.selCat === catId) return;
            this.selCat = catId;
            if (i !== undefined) {
                this.idx = i;
                this._applyColor(this.heroCats[i]?.bg_color || this.apiCats.find(c=>c.id===catId)?.bg_color || '#f97316');
            }
            this.selSvc = null; this.det = null; this.selPkg = null;
            this.loadingSvcs = true;
            try {
                const r = await fetch('/api/smart-booking/services?category_id='+catId);
                this.services = await r.json();
            } catch(e) { this.services = []; }
            this.loadingSvcs = false;
            if (this.services.length) await this.pickSvc(this.services[0]);
        },
        ...
        async pickSvc(svc) {
            ...
            try {
                ...
                this.det = {
                    ...
                    packages,
                    portfolio: this.buildPortfolio(svc),
        ...
        buildPortfolio(svc) {
            return (svc.portfolio_items||svc.portfolio||[]).map(item => {
                let thumb = null;
                if (item.image_path) thumb = item.image_path.startsWith('http') ? item.image_path : '/'+item.image_path;
                else if (item.youtube_video_id) thumb = 'https://img.youtube.com/vi/'+item.youtube_video_id+'/hqdefault.jpg';
                return { thumb, title:item.title||'', category:item.category_name||'' };
            });
        },
        ...
        sortedFeatures(pkg) {
            ...
            const common = feats.filter(f => shared.has(f)).map(f => ({ text: f, isExtra: false }));
            const extras = feats.filter(f => !shared.has(f)).map(f => ({ text: f, isExtra: true }));
