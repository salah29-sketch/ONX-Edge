...
class PortfolioItemsController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        } elseif ($request->input('media_type') === 'youtube' && $request->filled('youtube_url')) {
    ...
    public function update(Request $request, PortfolioItem $portfolioItem)
    {
        ...
        if ($isReel && $request->reel_source === 'youtube' && $request->filled('reel_url')) {
            $data['youtube_video_id'] = PortfolioItem::extractYoutubeVideoId($request->reel_url);
        } elseif ($isReel && $portfolioItem->reel_source === 'youtube' && $portfolioItem->reel_url) {
    ...
    private static function compressAndSave(\Illuminate\Http\UploadedFile $file, string $folder, string $name): string
    {
        ...
        $image->encode(new \Intervention\Image\Encoders\WebpEncoder(quality: 85))->save($savePath);
